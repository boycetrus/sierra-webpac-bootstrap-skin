...
$(function() {
  ...
  $('#sortdropdown')
    .before('<label for="sortdropdown" class="sr-only">Sort</label>')
    .unwrap('span');
  ...
  $('form#search > input[type="submit"]').wrapAll(
  ...
  $(
    'form[name="checkout_form"] .stacktable.small-only .patFuncHeaders'
  ).remove();
  $('form[name="checkout_form"] .stacktable.small-only input[type="checkbox"]')
    .wrap('<label></label>')
    .after(' Select');
  ...
  $('.patFuncStatus:contains("RENEW FAILED")')
    .closest('tr')
    .addClass('danger');
  $('.patFuncStatus:contains("RENEWED")')
    .closest('tr')
    .addClass('success');
  ...
  $('form[name="hold_form"] th.patFuncHeaders:contains("CANCEL IF NOT")').text(
    'EXPIRY DATE'
  );
  $('form[name="hold_form"] .st-key:contains("CANCEL IF NOT")').text(
    'EXPIRY DATE'
  );
  $('.patFuncFreezeLabel').addClass('sr-only');
  $('form[name="hold_form"] input[type="checkbox"]').on(
    'change',
    applyChangeReminder
  );
  ...
  $('h2:contains("FINES")')
    .next('table#patFunc')
    .addClass('patfunc-fines');
  $('.ecommerce-pay')
    .parents('a')
    .eq(0)
    .addClass('btn btn-primary')
    .insertBefore('table.patfunc-fines');
  $('.patfunc-fines .ecommerce-pay')
    .parents('a')
    .addClass('btn btn-primary')
    .insertAfter('table.patfunc-fines');
  ...
  $(
    '<nav class="reading-history-pagination text-center" aria-label="reading history pagination"><ul class="pagination"></ul></nav>'
  ).insertAfter('form[name="PHISTORYFORM"] table#patFunc');
  $('form[name="PHISTORYFORM"] .browsePager > span')
    .data('option', 'ICON_PAGING_CAPTION')
    .remove();
  $('form[name="PHISTORYFORM"] th.patFuncHeaders:contains("Mark")').text(
    'Select'
  );
  ...
  $('form[name="PHISTORYFORM"] ul.pagination')
    .children()
    .each(function() {
      if ($(this).is('strong')) {
        var pgNum = $(this).text();
  ...
  $('nav.reading-history-pagination')
    .clone()
    .insertBefore('form[name="PHISTORYFORM"] table#patFunc');
  $('form[name="PHISTORYFORM"] tr.browsePager').remove();
  $('form[name="PHISTORYFORM"] table').cardtable();
  $(
    'form[name="PHISTORYFORM"] table.stacktable.small-only:first-child'
  ).remove();

  // My Lists form
  $('form[name="mylists_form"] th.patFuncHeaders:contains("Mark")').text(
    'Select'
  );
  $('form[name="mylists_form"] table').cardtable();
  $(
    'form[name="mylists_form"] table.stacktable.small-only:first-child'
  ).remove();
  ...
  $('form[name="renameList"] .formButtonArea a:first-child').addClass(
    'btn btn-primary btn-xs'
  );
  $('form[name="renameList"] .formButtonArea a:nth-child(2)').addClass(
    'btn btn-default btn-xs'
  );
  $('form[name="newlistForm"] select').addClass('form-control');
  $('form[name="newlistForm"] select')
    .parent()
    .addClass('form-group');
  ...
  $('form[name="newlistForm"] .formButtonArea a:first-child').addClass(
    'btn btn-primary btn-xs'
  );
  $('form[name="newlistForm"] .formButtonArea a:nth-child(2)').addClass(
    'btn btn-default btn-xs'
  );

  // Preferred Searches Form
  $('form[name="PSEARCHFORM"] td.patFuncMark > input').wrap(
    '<div class="checkbox"><label></label></div>'
  );
  $('form[name="PSEARCHFORM"] td.patFuncPSrchType strong').wrapInner(
    '<h3></h3>'
  );
  $('form[name="PSEARCHFORM"] #patFunc')
    .removeClass('table table-bordered')
    .addClass('pref-search');
  $('form[name="PSEARCHFORM"] #patFunc tr.patFuncHeaders').remove();
  $('form[name="PSEARCHFORM"] #patFunc td').removeAttr('width');
  $(
    'form[name="PSEARCHFORM"] .patFuncEntry .patFuncMark:first-child label'
  ).append('Remove');
  $(
    'form[name="PSEARCHFORM"] .patFuncEntry .patFuncMark:nth-child(2) label'
  ).append('Email');
  $('form[name="PSEARCHFORM"] .patFuncPSrchBtn a').addClass(
    'btn btn-primary btn-xs'
  );
  $('form[name="PSEARCHFORM"] input[type="checkbox"]').on(
    'change',
    applyChangeReminder
  );
  ...
  $('td.browsePager')
    ...
    .removeAttr('colspan')
    .wrapInner('<ul class="pagination"/>');
  $('td.browsePager')
    .next('td')
    .remove();
  $('td.browsePager .pagination strong').wrap(
  ...
  $('.bib-title > h1').text($itemTitle);
  $('.bib-display-content .bibInfoLabel:contains("Summary")').parent('tr').appendTo('.bib-summary > table > tbody');
